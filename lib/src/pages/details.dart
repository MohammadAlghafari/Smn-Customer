...
import 'package:food_delivery_app/src/repository/chat_repository.dart';
...
class _DetailsWidgetState extends StateMVC<DetailsWidget> {
  ...
  void _restaurntChat() async {
    List<String> chatOwners = [];
    for (int i = 0; i < _con.restaurant.users.length; i++)
      chatOwners.add(_con.restaurant.users[i].id);
    ;
    chatOwners.add(currentUser.value.id);
    List<User> users = _con.restaurant.users.map((e) {
    ...
    Conversation _conversation =
        new Conversation(users, name: _con.restaurant.name, id: id);
    setState(() {
      widget.currentPage = ChatWidget(
          parentScaffoldKey: widget.scaffoldKey,
          routeArgument:
              RouteArgument(id: _con.restaurant.id, param: _conversation));
    });
  }
}
